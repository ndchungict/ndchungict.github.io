...
- Secret lấy từ **credential/env**, không hard-code ([Bài 10](../credentials-va-bao-mat-n8n/)).
...
> Lưu ý về raw body: ví dụ dùng `JSON.stringify($json.body)` cho gọn, nhưng trong thực tế **chữ ký phải tính trên đúng raw body** nhà cung cấp đã ký. Bật `rawBody` và lấy đúng chuỗi thô; stringify lại object đã parse có thể làm lệch chữ ký. Kiểm tra tài liệu HMAC của nhà cung cấp cụ thể.
